...
cur = con.cursor()
cur.execute('SELECT kinter from codes')
res = cur.fetchall()
...
def mmt():
  ...
  ts = int(time.time())
  url = f"https://flights-cb.makemytrip.com/api/search?pfm=PWA&lob=B2C&crId=24a02e9f-386c-4f9e-8972-43b6c1abc496&cur=INR&lcl=en&shd=true&cc=E&pax=A-1_C-0_I-0&it={fro}-{to}-{when_mmt}&forwardFlowRequired=true&apiTimeStamp='{ts}'&region=in&currency=inr&language=eng"
